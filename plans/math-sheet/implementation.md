# MathSheet — Hoja de Cálculo Matemático

## Goal

Reemplazar `Notepad.md`, `MathLiveNotepad.md` y `EjemploVigaACI.md` con un único archivo `MathSheet.md`: un canvas 2D con bloques arrastrables (drag nativo), input WYSIWYG por MathLive, evaluación top→bottom/left→right con math.js, y persistencia completa en localStorage.

## Prerequisites

Verificar que la rama activa sea `feature/math-sheet`. Si no existe, crearla:

```powershell
git checkout -b feature/math-sheet
```
...
#### Step 1: Eliminar archivos obsoletos y actualizar navegación

- [x] Verificar que estás en la rama correcta:

```powershell
git status
```

- [x] Eliminar los tres archivos obsoletos:

```powershell
Remove-Item "docs\herramientas\notepad-calculos\Notepad.md"        -Force
Remove-Item "docs\herramientas\notepad-calculos\MathLiveNotepad.md" -Force
Remove-Item "docs\herramientas\notepad-calculos\EjemploVigaACI.md"  -Force
```

- [x] En `mkdocs.yml`, localizar el bloque (4 líneas) que comienza con `- Notepad de Cálculos:` y **reemplazarlo completo** con:

```yaml
    - Notepad de Cálculos:
      - Introducción: herramientas/notepad-calculos/index.md
      - Hoja de Cálculo: herramientas/notepad-calculos/MathSheet.md
```

> **Contexto exacto del bloque a reemplazar en `mkdocs.yml`** (para no confundir líneas):
>
> ```yaml
>     - Notepad de Cálculos:
>       - Introducción: herramientas/notepad-calculos/index.md
...
- [x] Reemplazar **todo** el contenido de `docs/herramientas/notepad-calculos/index.md` con lo siguiente:
...
- [ ] Los archivos `Notepad.md`, `MathLiveNotepad.md` y `EjemploVigaACI.md` ya no existen en `docs/herramientas/notepad-calculos/`.
- [ ] Ejecutar `mkdocs serve` y comprobar que no aparecen errores de página no encontrada.
- [ ] El nav lateral muestra bajo "Notepad de Cálculos": sólo **Introducción** y **Hoja de Cálculo** (sin las entradas antiguas).
- [ ] La página de Introducción muestra sólo el enlace a `MathSheet.md`.

#### Step 1 STOP & COMMIT

**STOP & COMMIT:** Detener aquí. El usuario debe probar, preparar (`git add`) y confirmar (`git commit`) estos cambios antes de continuar.

---

#### Step 2: Crear MathSheet.md — implementación completa

- [x] Crear el archivo **`docs/herramientas/notepad-calculos/MathSheet.md`** con el siguiente contenido completo (no modificar nada):
...
- [ ] Abrir `mkdocs serve` y navegar a **Herramientas → Notepad de Cálculos → Hoja de Cálculo**.
- [ ] La página carga sin errores de consola relevantes (pueden aparecer advertencias de MathLive — son normales).
- [ ] El ejemplo precargado aparece: tres bloques de sección, cinco bloques de cálculo y un bloque de texto dispuestos en el canvas con cuadrícula visible.
- [ ] Los bloques de cálculo muestran resultados correctos:
  - `L = 6`, `wD = 15`, `wL = 10`
  - `wu = 34`  (= 1.2×15 + 1.6×10)
  - `Mu = 153` (= 34×36/8)
  - `Vu = 102` (= 34×6/2)
- [ ] Arrastrar un bloque por la cabecera → se mueve; al soltar encaja en múltiplos de 40 px.
- [ ] Desactivar checkbox **Cuadrícula** → arrastrar bloque → se queda donde se suelta (libre).
- [ ] Clic en **+ Cálculo** → aparece un nuevo bloque. Escribir `2+2` → resultado muestra `4`.
- [ ] Clic en **+ Texto** → aparece bloque con `<textarea>`.
- [ ] Clic en **+ Sección** → aparece bloque de sección estilado con borde naranja.
- [ ] Botón `×` del bloque (visible en hover) → elimina el bloque.
- [ ] Handle de resize (esquina inferior derecha, visible en hover) → arrastrarlo cambia el ancho del bloque.
- [ ] Botón **⌨ Teclado** → se activa (fondo azul); al hacer foco en un campo MathLive aparece el teclado virtual.
- [ ] Botón **⬇ JSON** → descarga un archivo `mathsheet.json`.
- [ ] Botón **Limpiar** → confirmar → canvas vacío; `localStorage` limpio.
- [ ] Botón **⬆ JSON** → seleccionar el JSON descargado → hoja restaurada.
- [ ] Recargar la página → el estado anterior se restaura desde `localStorage`.
- [ ] **Prueba de orden de evaluación**: crear bloque A (`x=5`) en y=200 y bloque B (`y=x+3`) en y=80. B (arriba de A) muestra error porque `x` no está definido aún. Mover A encima de B → B muestra `y = 8`. ✓
- [ ] **Modo oscuro**: alternar el tema → cuadrícula visible en gris claro sobre fondo oscuro; bloques con bordes y sombras visibles.

#### Step 2 STOP & COMMIT

**STOP & COMMIT:** Detener aquí. El usuario debe probar todos los puntos del checklist, preparar (`git add`) y confirmar (`git commit`) antes de continuar con funcionalidades futuras (bloques gráfico Plotly, etc.).
