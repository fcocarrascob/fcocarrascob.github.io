...
| 4 | Planchas, ángulos y canales soldados en el talón, tés y W con **soldaduras longitudinales** solamente | $\dfrac{3 l^2}{3 l^2 + w^2}\left(1 - \dfrac{\bar{x}}{l}\right)$ |
| 5 | **HSS circular o rectangular** con **una** plancha gusset concéntrica pasante por ranuras | $1 - \bar{x}/l$; en el rectangular $\bar{x} = b - \dfrac{2b^2 + tH - 2t^2}{2H + 4b - 4t}$ con $b = B/2$ |
| 6 | **HSS rectangular** con **dos** planchas gusset laterales | $U = \dfrac{B\,U_B + H\,U_H}{H+B}$, con $U_B = \dfrac{3l^2}{3l^2+B^2}$ y $U_H = \dfrac{3l^2}{3l^2+H^2}$ |
...
$D$ = diámetro del HSS circular, $t$ = espesor de diseño del HSS, $d$ = peralte del perfil,
$w$ = ancho de la plancha y $l_1, l_2$ ($\geq 4\times$ el tamaño de soldadura) las longitudes
de soldadura longitudinal.

<Note type="warning" title="Los casos 5 y 6 se renumeraron en 360-22">
Si vienes de **AISC 360-16**, ojo: allá el Caso 5 era solo el **HSS circular** y el Caso 6 el
rectangular con una plancha, con $\bar{x} = (B^2+2BH)/[4(B+H)]$ —una expresión que ignora el
espesor de la pared—. En **360-22** los dos HSS con una plancha pasante quedaron juntos en el
**Caso 5**, con una $\bar{x}$ que sí incorpora $t$ (y da un valor algo menor, o sea un $U$ algo
mayor), y el Caso 6 pasó a ser el tubo con **dos planchas laterales**. Citar «Caso 6» para un
tubo ranurado es la numeración de la edición anterior.
</Note>
...
No hay límite obligatorio de esbeltez para miembros en tracción — un tirante no pandea. La
cláusula D1 completa es una sola frase: *«There is no maximum slenderness limit for members in
tension.»* Con todo, su User Note **recomienda** que la esbeltez del miembro *tal como se
fabrica* —longitud fabricada dividida por el menor radio de giro— no exceda 300, para controlar
vibraciones y la flecha por peso propio durante el montaje, y aclara que **la sugerencia no
aplica a barras redondas**.
