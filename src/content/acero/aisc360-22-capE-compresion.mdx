...
description: "El Capítulo E de AISC 360-22 contado desde la estabilidad: la resistencia real de una columna no es Fy·Ag sino lo que el pandeo deja usar. La curva de columna (Fn), la longitud efectiva Lc = KL, los tres modos de pandeo global (flexión, torsional, flexo-torsional), el pandeo local y el orden de diseño."
pubDate: 2026-06-18
...
real llega ahí: **pandea antes**, con una tensión nominal $F_n < F_y$, y el pandeo es
súbito. Todo el Capítulo E es medir cuánto de ese techo $F_y A_g$ le roba la inestabilidad:

$$
P_n = F_n\,A_g \qquad\qquad \frac{F_n}{F_y} = \text{la fracción de } F_y \text{ que la columna alcanza a usar}
$$
...
<Note type="warning" title="Notación: F_n, no F_cr">
AISC 360-22 llama **$F_n$** ("nominal stress") a la tensión que sale de la curva de columna
—Ecs. E3-1, E3-2, E3-3, E4-1 y E7-1—. En **AISC 360-16** ese mismo valor se llamaba
$F_{cr}$. Documentación, planillas o software anteriores a 2022 usan la notación antigua;
...
<Figure
  ...
  caption="L_c = K·L. Empotrar los extremos acorta la longitud que 'cuenta' para el pandeo (baja K, sube F_n); un extremo libre la duplica. La misma columna rinde distinto según cómo se sujete."
/>

<Equation label="Sec. E2">
$$
L_c = K L
$$
</Equation>
...
Todo el contenido está en $F_n$, y la mejor forma de entenderla es como una curva de
$F_n$ contra la esbeltez $L_c/r$ — la **curva de columna**:

<Figure
  src="/aisc360-22-capE/curva-columna.svg"
  alt="Curva de F_n en función de la esbeltez Lc/r: una meseta cerca de Fy para columnas cortas, una transición inelástica, y una rama elástica que sigue 0.877 veces la curva de Euler para columnas esbeltas; la transición ocurre en Lc/r = 4.71 raíz de E/Fy"
...
rechonchas) $F_n$ roza $F_y$; a la derecha (columnas esbeltas) se desploma. **Bajar
...
El cálculo reusa la misma curva de columna (Ecs. E3-2/E3-3 para $F_n$), pero cambiando
...
área. La norma lo trata con el **método del ancho efectivo**: se calcula $F_n$ igual que
...
\qquad \text{si } \lambda > \lambda_r \sqrt{F_y / F_n}
$$
</Equation>
...
flowchart TD
    ...
    E --> F
    F --> G["5. F_n de la curva de columna<br/>E3-2 (inelástico) o E3-3 (elástico)"]
    G --> H["6. P_n = F_n·A (menor modo)<br/>φ_c·P_n ≥ P_u  (φ_c = 0.90)"]
```
...
| Pandeo por flexión | $F_n$ con $F_e$ de Euler (Sec. E3) | inestabilidad global — súbita |
| Pandeo torsional / flexo-torsional | $F_n$ con $F_e$ de E4 | inestabilidad global — súbita |
