...
Todos los demás capítulos de la norma responden la misma pregunta: *¿cuánto resiste esto?*
El Capítulo 18 responde otra: *¿cuánto se puede deformar antes de perder lo que resiste?*
...
- **18.2.8.1** — **no se permiten empalmes soldados** donde se espera fluencia.
...
con $\alpha_c = 0{,}25$ para $h_w/\ell_w \le 1{,}5$ y $\alpha_c = 0{,}17$ para
...
$$
\alpha_{sh} = 0{,}7\left(1 + \frac{(b_w + b_{cf})\,t_{cf}^{2}}{A_{cx}}\right) \le 1{,}2
$$
</Equation>
...
resistencia nominal a flexión, en la dirección del desplazamiento de diseño. Reordenada,
dice cuándo hace falta confinar:
...
El piso corta en dos direcciones. Un edificio rígido con deriva real de 0,002 debe
diseñarse *como si* tuviera 0,005 — y el umbral de $c$ baja a 2,5 veces. Pero una
estructura industrial chilena, donde NCh2369 6.3 admite hasta $0{,}015\,h$, va por el
camino contrario: su deriva real es **tres veces el piso**, y el umbral de $c$ baja otro
tanto. Es lo que hace que en Chile esta verificación deje de ser un trámite.
</Note>
...
- **(iii)** La verificación completa: $\delta_c/h_{wcs} \ge 1{,}5\,\delta_u/h_{wcs}$, con
...
Abdullah y Wallace (2019) sobre una base de ensayos de muros. Sus tres términos dicen tres
cosas físicas:
...
$A_{ch} = b_{c1}\,b_{c2}$. La expresión (a) de la tabla —la que depende de
...
## 8. Las tres novedades de 318-25 frente a 318-19

| Disposición | ACI 318-19 | ACI 318-25 |
|---|---|---|
| $\omega_v$ (Tabla 18.10.3.3.3) | Función del número de pisos $n_s$: $0{,}9 + n_s/10$ ($n_s \le 6$); $1{,}3 + n_s/30 \le 1{,}8$ | $0{,}8 + 0{,}09\,h_n^{1/3} \ge 1{,}0$, con $h_n$ en **pies** |
| $f'_c$ en la Ec. (18.10.4.1) | sin límite declarado sobre $\sqrt{f'_c}$ | **$f'_c \le 85$ MPa** |
| Topes de $V_n$ (18.10.4.4/5) | $0{,}66\sqrt{f'_c}A_{cv}$ y $0{,}83\sqrt{f'_c}A_{cw}$ | los mismos **multiplicados por $\alpha_{sh} \le 1{,}2$** |

---

## 9. El orden de diseño de un muro especial

```mermaid
flowchart TD
    A["1. Clasificar<br/>h_w/ℓ_w y ℓ_w/b_w<br/>(Tabla R18.10.1)<br/>¿muro o pilar de muro?"] --> B["2. ¿Puede ser ordinario?<br/>NCh2369 9.1.5: H_w/L_w &lt; 3<br/>con 0,7R₁ → se sale del 18.10"]
...
e instalaciones industriales*, Santiago, 2025 — cláusulas 6.3 (desplazamientos sísmicos
máximos) y 9.1.1/9.1.5 (diseño según NCh430 y la excepción de los muros de baja
esbeltez). El comentario C9.1.1 advierte que el **DS 60 (2011)** reemplazó a NCh430 y debe
considerarse mientras no sea derogado; el DS 60 agrega requisitos propios a los muros
chilenos que esta nota no cubre.
