...
<Equation label="Ec. 25.4.3.1(a)">
$$
\ell_{dh} = \left(\frac{f_y\,\psi_e\,\psi_s\,\psi_{cc}\,\psi_r}{21\,\lambda\sqrt{f'_c}}\right) d_b^{1.5}
\;\geq\; \max(8\,d_b,\ 150\ \text{mm})
$$
</Equation>

<Note type="warning" title="Cambió respecto a ACI 318-19">
ACI 318-19 escribía esta ecuación con denominador **23** y los factores
$\psi_e\,\psi_r\,\psi_o\,\psi_c$. ACI 318-25 usa denominador **21** y renombra los
factores a $\psi_e$ (epóxico), $\psi_s$ (tamaño de barra), $\psi_{cc}$ (recubrimiento)
y $\psi_r$ (refuerzo de confinamiento), definidos en 25.4.3.2. El cambio de 23 a 21
hace $\ell_{dh}$ un **≈ 10 % más largo**: usar la forma antigua queda del lado inseguro.
</Note>
...
proporcionalmente menos (para φ25, Gr 420, $f'_c = 25$ MPa y todos los $\psi = 1$:
$\ell_{dh} = 500$ mm $= 20\,d_b$; para φ36 ya va en $864$ mm $= 24\,d_b$). Los factores
$\psi_{cc}$ (recubrimiento) y $\psi_r$
(confinamiento) castigan el mismo modo de falla: el **splitting lateral** del plano del
gancho. Geometría estándar: extensión de
